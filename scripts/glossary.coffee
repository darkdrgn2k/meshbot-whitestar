...
module.exports = (robot) ->
  robot.respond /what is (.*)\??/i, (msg) ->
    ...
    items.push
        title:"Negative Voltage",content:"A negative voltage is really a positive earth potential. If your positive conductor i(+) is earth, you can't short it to earth. It can be shorted to the exchange earth connection if it comes into contact with a suitable conductor in the cable, but as this 'earth' is the negative battery terminal (technically) you don't get the massive current flow to earth for a conductor to earth. The only way you can get massive current flow is if you short the pair together or put the positive earth to a foreign wire connected to the negative battery terminal."
    items.push
        title:"Negative Voltage xkcd",content:"https://imgs.xkcd.com/comics/urgent_mission.png"
    items.push
        title:"markdown", content:"markdown is `code` and *italics* and **bold**"
    for i in [1...items.length]
        glossary=glossary + ', ' + items[i].title  
    items.push
        title:"glossary",content:"I know the following terms: " + glossary
    ...
    matches = results.map (el) ->
        el.original.content
    matchestitle = results.map (el) ->
        el.original.title

    matches = err_msg if matches.length is 0
    msg.send matchestitle[0] + ' is '
    msg.send matches[0]

    if matches.length==2
        msg.send matchestitle[1] + ' is '
        msg.send matches[1]
